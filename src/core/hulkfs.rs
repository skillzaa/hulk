...
impl HulkFs {
  ...
  pub fn create_dir(&self,dir_name:&str)->std::io::Result<()> {
    let path = String::from("./") + &dir_name;
  
    let d = fs::create_dir(path)?;
    Ok(d)
  }
  pub fn create_dir_all(&self,dir_name:String)->std::io::Result<()> {
    //prepend the ./
    let path = String::from("./") + &dir_name;
    println!("path {}",path);
  ...
  pub fn read_dir (&self,dir_name:&str)->Vec<DirEntry>{
  ...
  pub fn get_dir_from_dir (&self,dir_name:&str)->Vec<DirEntry>{
    let mut v:Vec<DirEntry> = Vec::new();
        for entry in fs::read_dir(dir_name).unwrap() {
          let entry = entry.unwrap();
          let path = entry.path();
  ...
  pub fn get_files_from_dir (&self,dir_name:&str)->Vec<DirEntry>{
    let mut v:Vec<DirEntry> = Vec::new();
        for entry in fs::read_dir(dir_name).unwrap() {
          let entry = entry.unwrap();
          let path = entry.path();
  ...
  pub fn write_to_file(&self,file_name:&str,content:&str) -> std::io::Result<()> {
  ...
  pub fn get_files_by_ext (&self,dir_name:&str,ext:&str)->Vec<DirEntry>{
    let mut v:Vec<DirEntry> = Vec::new();
        for entry in fs::read_dir(dir_name).unwrap() {
          let entry = entry.unwrap();
          let path = entry.path();
